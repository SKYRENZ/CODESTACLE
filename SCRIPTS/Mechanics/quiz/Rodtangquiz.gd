...
func resize_quiz() -> void:
	...
	vbox.position.x = (content_rect.size.x - vbox.size.x) / 2  +90
	...
	var runi_sprite = $CanvasLayer/ColorRect2/ColorRect/Emcinspired
	runi_sprite.position.x = 110
	runi_sprite.position.y = content_rect.size.y - 40
	runi_sprite.scale = Vector2(0.045, 0.045)  # Smaller scale
